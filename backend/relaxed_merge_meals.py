...
def main():
            ...
            if len(union) > 0:
                ...
                if option_a or option_b:
                    if not has_conflict(old_words_raw, new_words_raw):
                        match_found = True
                        matched_with = new_name
                        match_type = f"SEMANTIC (TFIDF={tfidf_sim:.2f}, Jac={jaccard:.2f}, Int={len(intersection)})"
    ...
    with open("final_balanced_meals.json", "w", encoding="utf-8") as f:
        json.dump(final_list, f, indent=2)
    print("\nSaved final_balanced_meals.json")
